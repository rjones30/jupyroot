#!/usr/bin/env python3
#
# hddmview - python class that acts as a container of ROOT (see https://root.cern.ch)
#            objects for statistical visualization of data contained in hddm stream
#            files (see https://github.com/rjones30/HDDM). Users define the ROOT
#            histograms that visualize the data in interesting ways, together with
#            methods for filling those histograms from the contents of hddm records,
#            and identify those histograms with a unique user-assigned string name.
#            The hddmview object fills the histograms by scanning through the hddm
#            files provided by the user, and caches the result for future reference.
#            Methods for displaying the histograms in a jupyter notebook are also
#            provided.
#
# author: richard.t.jones at uconn.edu
# version: september 12, 2024
# notes: See "pydoc jupyroot.hddmview" for details of the API.
...
class hddmview:
   ...
   def __init__(self, inputfiles, hddmclass, savetoroot):
      """
      ...
      if os.path.isfile(self.savetorootfile):
         ...
         f.mkdir(self.savetorootdir)
      f.Close()
      self.hddmclass = hddmclass
      try:
         for rec in hddmclass.istream(inputfiles[0]):
            break
      except:
         raise ValueError("hddmview constructor error -",
                          "first element of inputfiles is not a valid",
                          "hddm file of class", hddmclass)
      self.histodefs = {}
   ...
   def declare_histograms(self, setname, initfunc, fillfunc):
      ...
      for hname,histo in initfunc().items():
         if hname != histo.GetName():
            print("hddmview.declare_histograms error -",
